...
char *any_(void *restrict res, const int typeof_res, const void *restrict value_var,
  ...
  LOOP_OVER_ROWS {
    if (OUTPUT_I == TRUE) continue;
    if (input[i] == NA_INTEGER) {
      if (na_rm) continue;
      if (OUTPUT_I == FALSE) OUTPUT_I = NA_INTEGER;
    } else if (input[i]) {
      OUTPUT_I = TRUE;
    }
    if (!HIT_I) HIT_I = 1;
